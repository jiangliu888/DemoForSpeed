import {deleteCompanyByName, deleteAllSites, createCompanyByData, createSiteByData, getLogsNumber} from '../utils/basic-utils'
...
describe('firewall page test', function() {
   beforeEach(function () {
    ...
    it('create modify and delete firewall test', function(){
      ...
      getLogsNumber(this.info.token,{resourceType:'acl',action:'add',company:this.company.testCompany1Firewall.name,username:'admin'})
      cy.get('@logsNumber').then(logsNumber =>{expect(logsNumber).to.equal(1)})
      getLogsNumber(this.info.token,{resourceType:'acl',action:'del',company:this.company.testCompany1Firewall.name,username:'admin'})
      cy.get('@logsNumber').then(logsNumber =>{expect(logsNumber).to.equal(1)})
      getLogsNumber(this.info.token,{resourceType:'acl',action:'update',company:this.company.testCompany1Firewall.name,username:'admin'})
      cy.get('@logsNumber').then(logsNumber =>{expect(logsNumber).to.equal(2)})
      getLogsNumber(this.info.token,{resourceType:'acl',action:'update',company:this.company.testCompany1Firewall.name,username:'admin',detail:'应用防火墙规则'})
      cy.get('@logsNumber').then(logsNumber =>{expect(logsNumber).to.equal(1)})
    })
