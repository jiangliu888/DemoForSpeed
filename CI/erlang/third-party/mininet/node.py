...
- Possibly make Node, Switch and Controller more abstract so that
  they can be used for both local and remote nodesf
...
class IVSSwitch( Switch ):
    ...
    def __init__( self, name, verbose=False, **kwargs ):
        ...
        self.port_num = kwargs.pop('port_num')
        self.use_group = kwargs.pop('use_group', False)
        print self.port_num, self.use_group

    @classmethod
    def setup( cls ):
        "Make sure docker is installed"
        out, err, exitcode = errRun( 'docker --version' )
        if exitcode:
            error( out + err +
                   'dockers exited with code %d\n' % exitcode +
                   '*** The docker might not be installed or '
                   'not be loaded. Try modprobe dockers.\n' )
            exit( 1 )

    @classmethod
    def batchShutdown( cls, switches ):
        "Kill each ai switch, to be waited on later in stop()"
        delete_all_ne()
        for switch in switches:
            switch.shell = None
        return switches
    ...
    def start( self, controller):
        "Start up a new ai switch"
        start_aiswitch(self.name, controller[0].IP(), controller[0].port, self.port_num, self.use_group)


    def stop( self, deleteIntfs=True ):
        """Terminate ai switch.
           deleteIntfs: delete interfaces? (True)"""
        stop_aiswitch(self.name)

    def attach( self, intf ):
        "Connect a data port"
        print 'attach{}'.format(intf)
        pass
